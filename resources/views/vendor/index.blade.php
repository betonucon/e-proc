        ...
        var handleDataTableFixedHeader = function() {
            ...
            if ($('#data-table-fixed-header').length !== 0) {
                var tables=$('#data-table-fixed-header').DataTable({
                    ...
                    responsive: true,
                    ajax:"{{ url('vendors/get_data')}}?tanggal=",
					columns: [
...
<div id="content" class="content">
    <ol class="breadcrumb float-xl-right">
        ...
        <li class="breadcrumb-item active">Pengajuan Rekanan</li>
    </ol>
    <h1 class="page-header">Pengajuan Rekanan <small></small></h1>
    
    <div class="row" id="show-dashboard">
       
    </div>
    <div class="btn-group" style="margin-bottom:1%">
        <button class="btn btn-sm btn-success" onclick="location.assign(`{{url('vendors/create')}}?id={{encoder(0)}}`)"><i class="fas fa-plus"></i> Vendor Baru </button>
    </div>
...
@push('ajax')
   <script>
        $('#show-dashboard').load("{{url('vendors/show_dashboard')}}");
